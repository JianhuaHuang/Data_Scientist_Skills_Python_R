...
---
# Job skills recommendation system over space
This project is proposed for the Data Incubator Challenge. 
...
## Data source
This proposed project mainly depends on the data scraped from indeed.com. For a given type of job, the job title, job location, and job link will be scraped from indeed.com. With the job links, we can scraped the job description in each job page and extract all of the visible text from it. The visible text will be processed to removed some punctuation and split into words. These words will be used to generate word clouds and extract the requried skills. 

## Preliminary example
As an example, I will use "data scientist" as the given job type to generate the spatial distribution of jobs and the word cloud of required skills. I will use Python to scrape the web data from indeed.com and use R to generate the plots

### Web scraping with Python
This part explain the process of web scraping with Python. 

1.Here is the modules required for this project. 
```{r,eval=FALSE}
# -*- coding: utf-8 -*-
import bs4 
import urllib
...
```{r,eval=FALSE}
cd 'C:\\Users\\temp\\Dropbox\\test\\DataIncubator'
...
* Remove useless information (e.g., stop words, and punctuations) to generate clean text. 
...
### Plots with R
This part explains the processes of ploting the spatial job distribution, word cloud, and required skills in R. It requires some steps to extract some useful information and reformating, so that they can be used as input data for plots. These steps include:
